...
var RouteStore = Fluxxor.createStore({
 ...
 handleDimensionUpdate: function (payload) {
   ...
   var query = this.router.getCurrentQuery()

   if (query[dimension] !== value || dimension !== 'd1' ) {
     //dimension is changing!
     this.handleFieldReplace(payload)
   }
  //  else {
  //    //d1 dimension:value is already present -> do nothing
  //  }
 },

 handleFieldReplace: function (payload, multiple) {
 ...
 handleDimensionDependencies: function (query) {
   log('handleDimensionDependencies', query )
   if (query.d1 !== this.router.getCurrentQuery().d1) {
     //new d1
     delete query.d2
     delete query.d3
   }
   if (!this.router.getCurrentQuery().d1 && query.d2) {
     //set d1 from breakdown default
     var indicatorSlug = this.getCurrentParams().indicatorSlug
     var indicator = this.flux.stores.indicators.findOne({ slug: indicatorSlug })
     var breakdownIds = indicator.breakdowns.split(',')
     query.d1 = breakdownIds[0]
   }
