    ...
    if (dateOptions.length > 1) {
      ...
      DatasetSelect = (
        <DropdownButton bsStyle={'link'} title={selectedOption.label} key={0}>
          {
            _.map(dateOptions, function (option, i) {
      ...
      DatasetSelect = (
        <div className="single-dataset-label">
          {selectedOption.label}
        </div>
      )
    }

      if (dateOptions.length > 1 && this.props.indicator.datatype !== 'categorical') {
        ViewSelect = (
          [
      ...
      return (
        <div className="hrc-filter hrc-filter-dataset">
          ...
          <div key={'date-filter-group'} className='date-filter-group'>
            { ViewSelect }
          </div>
          {
