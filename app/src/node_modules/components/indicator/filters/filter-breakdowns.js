...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var dimensions = (
      <Row className='hrc-filter-breakdown'>
        {
          _.map(this.props.breakdownOptions,function(dimension, dim_index, breakdownOptions){
            var className = "filter-breakdown-"+dim_index
            var dimensionTitle = (breakdownOptions.length > 1 && !isCategorical(breakdownOptions[dim_index])) ?
              (<p className='filter-breakdown-title small'>{ordinal(dim_index+1) + ' dimension'}</p>) :
              null
            return (
              <Col className={className} xs={3} md={3} >
                {dimensionTitle}
                <ListGroup className>
                  {
    ...
    return (
      <div className='filter-breakdowns'>
        <div className='filter-breakdowns-header'>
          <h4 className='text-uppercase'>explore data by <strong className='emphasis'>by breakdown</strong></h4>
        </div>
        {dimensions}
      </div>
    )
