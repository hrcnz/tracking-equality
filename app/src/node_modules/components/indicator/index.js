...
//     log('data - filtered', data)
      ...
      howToReadData = this.truncateText(indicator.howtoreaddata, 45, !this.state.explanationCollapsed)
      aboutIndicator = this.truncateText(indicator.aboutindicator, 25, !this.state.explanationCollapsed)
...
//          }
  ...
  truncateText: function (text, limit, openState) {
    var overflow = false
    var showLink = false
    var truncated = text.split(' ')
    if (truncated.length > limit) {
      showLink = true
      if (!openState) { overflow = true  }
    }
    if (!openState) {
      truncated = truncated.slice(0, limit).join(' ')
    }
    else { truncated = truncated.join(' ') }
