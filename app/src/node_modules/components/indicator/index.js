...
//     log('data - filtered', data)
      ...
      howToReadData = this.state.explanationCollapsed ? this.truncateText(indicator.howtoreaddata, 45) : indicator.howtoreaddata
      aboutIndicator = this.state.explanationCollapsed ? this.truncateText(indicator.aboutindicator, 20) : indicator.aboutindicator

      ReadMore = (
        <div>
          <a onclick={this.toggleExplanation}>{this.state.explanationCollapsed ? 'Show more' : 'Show less'}</a>
        </div>
      )
            ...
            <Grid >
              ...
              <Row className='indicator-explain'>
                <Col className='indicator-title' md={9}>
                  ...
                  { (howToReadData.overflow) ? ReadMore : null }
                </Col>
                <Col className='indicator-definitions' md={3}>
                  ...
                  { (aboutIndicator.overflow) ? ReadMore : null }
                </Col>
              </Row>
              <hr/>
...
//          }
  ...
  truncateText: function (text, limit) {
    ...
    if (truncated.length > limit) overflow = true
    truncated = truncated.slice(0, limit).join(' ')
    if (overflow) truncated += '...'
    return {text: truncated, overflow: overflow}
  },
