...
//components
var Explore 						= require('./filters/explore')
var TimeSelect 					= require('./filters/time-select')
  ...
  render: function () {
    ...
    var ranges = []
      ...
      dataSetFilter = {}
          ...
          activeDatasets = [this.props.query.dataset]
      ...
      dataGroupsByActiveBreakdowns = this.extractDataGroups(data, activeBreakdownIds)
      
      log('dataGroupsByDimension',dataGroupsByActiveBreakdowns)
...
//						breakdowns={breakdownsActive}
//            groups={this.filterDataGroupsByQuery(dataGroupsByDimension, dataQuery)}
...
//
	...
	extractDataGroups: function (data, dimensions) {
		...
		_.each(dimensions, function (dimension) {
			var dgs
			if (dimension === 'age') {
				dgs = this.extractAgeGroups(data)
			}
			else if (dimension === 'cat') {
				dgs = this.extractCatGroups(data)
			}
			else {
        dgs = this.extractBreakdownGroups(data,dimension)								
			}
  ...
  extractBreakdownGroups : function(data,breakdown){
    var groupIds = _.without(_.uniq(_.pluck(data,breakdown)),'all')
    
    return _.map(groupIds,function(groupId){
      return this.props.flux.stores.dataGroups.findOne({ breakdown: breakdown, short: groupId })      
    },this)
    
  },
  extractAgeGroups : function(data){
    var breakdown = 'age'
    var groupIds = _.without(_.uniq(_.pluck(data,breakdown)),'all')
  ...
  extractCatGroups : function(data){
    var breakdown = 'cat'
    return _.without(_.uniq(_.pluck(data,breakdown)),'all')    
  },
    ...
    if(indicator.multidimensional === 'n'){
      ...
      dim.push({
        ...
        title : _.findWhere(breakdowns,{id:activeBreakdownIds[0]}).title
      })
      ...
      _.each(breakdownIds,function(bdid_and,index){
        ...
        if(index <= _.without(activeBreakdownIds,'cat').length ) {
          ...
          _.each(breakdownIds,function(bdid_or){
    ...
    log('breakdownsRequired',breakdownsRequired)    
    ...
    log('activeBreakdowns 1',activeBreakdowns)
    // TODO check for gap larger than 1
    // fill gaps with required breakdowns
    var keys = _.keys(activeBreakdowns).sort()
    for(var i = 1; i < keys.length; i++) {
      if(parseInt(keys[i]) - parseInt(keys[i-1]) !== 1) {  
        for(var j = parseInt(keys[i-1]); j < parseInt(keys[i]); j++) {
         activeBreakdowns[parseInt(keys[j-1])+1] = breakdownsRequired.shift()
        }
      }
    }
    log('activeBreakdowns 2',activeBreakdowns)
    log('breakdownsRequired 2',breakdownsRequired)    
    ...
    log('activeBreakdowns 3',activeBreakdowns)
