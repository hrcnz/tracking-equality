...
module.exports = React.createClass({
  ...
  render: function () {
		...
		//set the View and load up with its data
			...
			sortedDatasets = getDatasets(data,flux, dataQuery)
			...
			//remove query fields that are not breakdownIds or 'dataset'
			dataQuery = _.pick(dataQuery, function (val, key) {
				return _.contains(breakdownIds, key) || key === 'dataset'
			})
			...
			dataGroups = this.getDataGroups(breakdownIds, data)
	...
	getDataGroups: function (breakdownIds, data) {
		if (this.props.flux.stores.dataGroups.isLoading()) return []
		var ageGroups = _(data)
			.map(function (d) {
				return _.trim(d.age)
			})
			.uniq()
		...
		log('ageGroups', ageGroups)
		return this.props.flux.stores.dataGroups
