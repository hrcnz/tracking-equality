  ...
  render: function () {
    ...
    var ranges = []
		if (this.state.indicatorsLoading || this.state.dataLoading || this.state.issuesLoading) {
          ...
          currentDatasets = sortedDatasets		          
				} else {
          currentDatasets = [this.props.query.dataset]
          dataSetFilter.dataset = this.props.query.dataset
        }
			}
			else {
				currentDatasets = [sortedDatasets[0].id]
        dataSetFilter.dataset = sortedDatasets[0].id
			}
      //get data by dataset
			currentData = _(data).reject({ value: '' }).filter(dataSetFilter).value()            
      // get ranges (top/bottom) for each dataset
      var ranges = _(currentDatasets).map(function(dataset){
        return getRange(_(currentData).filter({dataset:dataset}).value(),indicator)
      }).value()
			...
			filteredByDimensionsAndQuery = this.filterByQueryWithAll(filteredByDimensions, dataQuery)
	...
	filterByQueryWithAll: function (data, query) {
	...
	componentWillRecieveProps: function (nextProps) {
		...
		if (!this.state.indicatorsLoading && !nextProps.query.d1 && nextProps.params.indicatorSlug && nextProps.flux) {
	...
	getShorts: function (data, breakdownId) {
		return _.keys(_.reduce(data, function (memo, d) {
			if (!memo[d[breakdownId]] && d[breakdownId] !== 'all') {
				log('d[breakdownId]', d[breakdownId])
	...
	getBreakdowns: function (breakdownIds) {
		...
		var first = this.props.query.d1 || this.state.defaultD1
	...
	getDimensions: function (query, breakdownIds) {
		...
		if (!query.d1) dimensions.push(this.state.defaultD1)
		return dimensions
	},
