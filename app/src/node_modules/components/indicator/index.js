...
module.exports = React.createClass({
  ...
  getStateFromFlux: function () {
    ...
    return {
      ...
      bdLoading : flux.store('dataBreakdowns').isLoading()      
    }
  },

  componentWillReceiveProps: function (nextProps) {
    //collapse explanation when switching between indicators
    if (!this.state.explanationCollapsed && nextProps.params.indicatorSlug !== this.props.params.indicatorSlug) {
      this.setState({ explanationCollapsed: true })
    }
  },
  ...
  render: function () {
    ...
    if (this.state.indicatorsLoading 
            || this.state.dataLoading 
            || this.state.bdLoading 
...
//     log('data - filtered', data)
      ...
      //truncate the explanation text if too long and explanation is collapsed
      howToReadData = truncateText(indicator.howtoreaddata, 60, !this.state.explanationCollapsed)
      aboutIndicator = truncateText(indicator.aboutindicator, 25, !this.state.explanationCollapsed)

      ShowMore = (
        <div className="show-more">
          <a href="#" onClick={this.toggleExplanation}>{this.state.explanationCollapsed ? 'Show more' : 'Show less'}</a>
        </div>
      )

      return (
        <div className="page-wrapper page-wrapper-indicator">
          <NavPrimary
            flux = {flux}
            type = 'issue'
            items = {
          ...
          />
          <div className="page-content-indicator page-content">
          
            <Grid className='page-section indicator-section-intro'>
              ...
              <Row className='indicator-top'>
                <Col className='indicator-explain' md={9}  xs={9}>
                  <h5>How to read the data</h5>
                  <div className='indicator-explain-text marked-inner-html' dangerouslySetInnerHTML={{
                    __html: marked(howToReadData.text, {sanitize: true})
                  }} ></div>
                  { (howToReadData.showLink) ? ShowMore : null }
                </Col>
                <Col className='indicator-definitions' md={3} xs={3}>
                  <h5>About this indicator</h5>
                  <div className='small indicator-definitions-text marked-inner-html' dangerouslySetInnerHTML={{
                    __html: marked(aboutIndicator.text, {sanitize: true})
                  }} ></div>
                  { (aboutIndicator.showLink) ? ShowMore : null }
                </Col>
              </Row>
             
            </Grid>
            
            <Grid className='page-section-spacer'><hr/></Grid>
            
            <Grid className='page-section indicator-section-chart'>
              <Row className='indicator-chart-top'>
                <Col className='indicator-dimension-filters' md={9} xs={9}>

                      { this.renderFilterBreakdowns(breakdownOptions) }                

                </Col>
                <Col className='indicator-dataset-filter' md={3} xs={3}>
                  <FilterDataset
                    flux={this.props.flux}
                    indicator={indicator}
                    dateOptions={this.getDateOptions(sortedDatasets,activeDatasets)}
                    activeView={this.props.query.dataset === 'all' ? 'timeline' : 'snapshot'}
                    timelineOption={sortedDatasets.length > 1} />
                </Col>
              </Row>
              <Row className='indicator-chart-main'>

                <Col className='indicator-chart' md={9} xs={9}>
                  {ChartComponent}
                </Col>

                <Col className='indicator-group-filter' md={3} xs={3}>
                  <FilterGroups
                    flux={this.props.flux}
            ...
            <Grid className='page-section indicator-section-analysis'>
              <Row className="indicator-bottom">
               <Col md={9} xs={9} className='indicator-analysis'>
                  <h2>Analysis</h2>
                  <div
                    className="indicator-commentary marked-inner-html"
                    dangerouslySetInnerHTML={{
                      __html: marked(indicator.commentary, {sanitize: true})
                    }}
                  />
                </Col>
                <Col className="indicator-sources" md={3} xs={3}>
  ...
  extractDataGroups: function (query, data, activeBreakdownIds, categories) {
    ...
    log('activeBreakdownIds', activeBreakdownIds)
    _.each(activeBreakdownIds, function (activeBreakdownId) {
      var dgs
      if (activeBreakdownId === 'age') {
        dgs = this.extractAgeGroups(query, data)
      }
      else if (activeBreakdownId === 'cat') {
        dgs = this.extractCatGroups(query, data, categories)
      }
      else {
        dgs = this.extractBreakdownGroups(query, data, activeBreakdownId)
      }
  ...
  extractBreakdownGroups : function(query, data, breakdownId){
    var groupIds = _.without(_.uniq(_.pluck(data, breakdownId)),'all')
    ...
    return _(groupIds)
      .map(function(groupId){
        ...
        group.active = this.isGroupInQuery(query, breakdownId, groupId)
        return group
      },this)
      .sortBy('rowNumber') //display order matches row order in spreadsheeet
      .value()
...
//      })
      ...
      _.each(allBreakdownIds,function(bdid){

          dim.push({
            active : (activeBreakdownIds.indexOf(bdid) > - 1),
            id:bdid,
            optional : bdid === 'cat' ? false : true,
            title : bdid === 'cat'
                  ? indicator.categorylabel
                  : _.findWhere(breakdowns,{id:bdid}).title
            })
...
//          }
          ...
          _.each(allBreakdownIds,function(bdid_or){
            if(actives.indexOf(bdid_or) === -1 ){
              dim.push({
                active : (activeBreakdownIds[index] === bdid_or),
  ...
  toggleExplanation: function (e) {
    e.preventDefault()
    log('toggleExplanation')
    this.setState({ explanationCollapsed: !this.state.explanationCollapsed })
  }
})
