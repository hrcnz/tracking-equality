...
module.exports = React.createClass({
	...
	mixins: [ FluxMixin, StoreWatchMixin('indicators') ],
  ...
  render: function () {
		...
		//set the View and load up with its data
			...
			referenceGroup = _.zipObject(breakdownIds, _.fill(Array(breakdownIds.length), 'all')) // serach data for {key: 'all', key: 'all'}
			...
			View = (
				<div>
					...
					<Snapshot
						...
						indicator={indicator}
						options={this.getDateOptions(sortedDatasets)}/>
				</div>
			)
		}
	...
	getBreakdowns: function (breakdownIds) {
		...
		return _(breakdowns)
			.sortBy(function(breakdown) {
				return this.state.cellOrder[breakdown.id]
			}.bind(this))
			.map(function (breakdown) {
				var isActive = (this.props.query[breakdown.id] && isArray(this.props.query[breakdown.id]))
					? !_.contains(this.props.query[breakdown.id], 'all')
					: (this.props.query[breakdown.id] !== 'all')

				return extend(breakdown, { isActive: isActive })
			}.bind(this))
			.value()
	},
