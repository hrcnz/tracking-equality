...
module.exports = React.createClass({
        ...
        <div className='snapshot'>
          <table className='table chart-table'>
            <TableHeader
              ...
              referenceDatum  = {refDatum}
							dataType 				= {this.props.indicator.datatype}
            />
            { (this.props.sortBy === 'groups') ? this.renderByGroup(refDatum.value) : this.renderByValue(refDatum.value) }
          </table>
        </div>
      )
  },
  ...
  prepareRowData : function (datum, maxValue, refValue) {
    maxValue = typeof maxValue !== 'undefined' ? maxValue : 1
    // one cell for each active dimension
    return _(this.props.breakdowns).map(function(bd){
        // get group cells
        return this.prepareLabelCell (makeGroupLabel(this.props.flux,[bd.id],datum) )
      }.bind(this)).value()
      // finally the datum cell
      .concat(this.prepareBarCell(datum.value, maxValue, refValue))
  },
