...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var interpolated = this.flatten(_.map(this.props.lineData, function (group) {
      ...
      interpolatedGroup.data = this.interpolate(group.data, 3)
  ...
  interpolate: function (timeseries, n) {
    ...
    for (var i = 0; i < timeseries.length; i++) {
      var prev = interpolated[0]
      var point = timeseries[i]
      if (prev) {
        var prevUnix = prev.date.valueOf()
        var thisUnix = point.date.valueOf()
        var timeStep = (thisUnix - prevUnix) / (n + 1)
        var valueStep = (point.y - prev.y) / (n + 1)

        for (var j = 0; j < n; j++) {
          interpolated.unshift({
            groupProperties: prev.groupProperties,
            date: new Date(prevUnix + (timeStep * (j+1))),
            y: prev.y + (valueStep * (j+1))
          })
        }
      }
      interpolated.unshift(point)
    }
